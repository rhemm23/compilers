  ...
  public void analyze(SymTable symbolTable) {
    if (symbolTable.lookupLocal(id.getValue()) == null) {
      ...
      for (DeclNode declaration : declarations) {
        ...
        sym.addMember(
          varDecl.getIdNode().getValue(),
          varDecl.getTypeNode().getType()
        );
      }
      symbolTable.addDeclaration(id.getValue(), sym);
    } else {
...
class StructNode extends TypeNode {
  ...
  public void analyze(SymTable symbolTable) {
    if (symbolTable.lookupGlobal(id.getValue()) == null) {
...
class StringLitNode extends ExpNode {
  ...
  public IntLitNode(int lineNum, int charNum, String value) {
    this.charNum = charNum;
    this.lineNum = lineNum;
    this.value = value;
  }
...
class DotAccessExpNode extends ExpNode {
  ...
  public void analyze(SymTable symbolTable) {
    ...
    if (sym == null || !(sym instanceof StructSymbol)) {
      leftId.reportError("Dot-access of non-struct type");
    } else {
      StructDefinitionSymbol structSym = symbolTable.lookupGlobal(((StructSymbol)sym).getStructId().getValue());
